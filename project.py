...
def inverse_warp(img, depth, egomotion, intrinsic_mat, intrinsic_mat_inv):
  ...
  source_pixel_coords = tf.transpose(a=source_pixel_coords, perm=[0, 2, 3, 1])
...
def _bilinear_sampler(im, x, y, name='blinear_sampler'):
  ...
  with tf.compat.v1.variable_scope(name):
    ...
    # Constants.
    batch_size = tf.shape(input=im)[0]
...
def _spatial_transformer(img, coords):
  ...
  img_height = tf.cast(tf.shape(input=img)[1], tf.float32)
  img_width = tf.cast(tf.shape(input=img)[2], tf.float32)
  px = coords[:, :, :, :1]
  py = coords[:, :, :, 1:]
...
def get_cloud(depth, intrinsics_inv, name=None):  # pylint: disable=unused-argument
  ...
  with tf.compat.v1.name_scope(name):
    ...
    cam_coords = tf.transpose(a=cam_coords, perm=[0, 2, 1])
