...
class Model(object):
  ...
  def build_inference_for_training(self):
    ...
    with tf.compat.v1.name_scope('egomotion_prediction'):
      self.egomotion, _ = nets.egomotion_net(self.image_stack, is_training=True,
                                             legacy_mode=self.legacy_mode)
    with tf.compat.v1.variable_scope('depth_prediction'):
  ...
  def build_loss(self):
    ...
    with tf.compat.v1.name_scope('compute_loss'):
      ...
      for s in range(NUM_SCALES):
        ...
        self.images[s] = tf.image.resize(self.image_stack,
                                              [height_s, width_s], method=tf.image.ResizeMethod.AREA)
        ...
        for i in range(self.seq_length):
          for j in range(self.seq_length):
            ...
            self.reconstr_loss += tf.reduce_mean(
                input_tensor=self.warp_error[s][key] * self.warp_mask[s][key])
            # SSIM.
            ...
            if self.icp_weight > 0:
              ...
              self.icp_transform_loss += 1.0 / (2**s) * tf.reduce_mean(
                  input_tensor=tf.abs(self.icp_transform[s][key]))
              self.icp_residual_loss += 1.0 / (2**s) * tf.reduce_mean(
                  input_tensor=tf.abs(self.icp_residual[s][key]))
  ...
  def build_train_op(self):
    with tf.compat.v1.name_scope('train_op'):
      optim = tf.compat.v1.train.AdamOptimizer(self.learning_rate, self.beta1)
      self.train_op = slim.learning.create_train_op(self.total_loss, optim)
      self.global_step = tf.Variable(0, name='global_step', trainable=False)
      self.incr_global_step = tf.compat.v1.assign(self.global_step, self.global_step + 1)

  def build_summaries(self):
    """Adds scalar and image summaries for TensorBoard."""
    tf.compat.v1.summary.scalar('total_loss', self.total_loss)
    tf.compat.v1.summary.scalar('reconstr_loss', self.reconstr_loss)
    if self.smooth_weight > 0:
      tf.compat.v1.summary.scalar('smooth_loss', self.smooth_loss)
    if self.ssim_weight > 0:
      tf.compat.v1.summary.scalar('ssim_loss', self.ssim_loss)
    if self.icp_weight > 0:
      tf.compat.v1.summary.scalar('icp_transform_loss', self.icp_transform_loss)
      tf.compat.v1.summary.scalar('icp_residual_loss', self.icp_residual_loss)

    for i in range(self.seq_length - 1):
      tf.compat.v1.summary.histogram('tx%d' % i, self.egomotion[:, i, 0])
      tf.compat.v1.summary.histogram('ty%d' % i, self.egomotion[:, i, 1])
      tf.compat.v1.summary.histogram('tz%d' % i, self.egomotion[:, i, 2])
      tf.compat.v1.summary.histogram('rx%d' % i, self.egomotion[:, i, 3])
      tf.compat.v1.summary.histogram('ry%d' % i, self.egomotion[:, i, 4])
      tf.compat.v1.summary.histogram('rz%d' % i, self.egomotion[:, i, 5])

    for s in range(NUM_SCALES):
      for i in range(self.seq_length):
        tf.compat.v1.summary.image('scale%d_image%d' % (s, i),
                         self.images[s][:, :, :, 3 * i:3 * (i + 1)])
        if i in self.depth:
          tf.compat.v1.summary.histogram('scale%d_depth%d' % (s, i), self.depth[i][s])
          tf.compat.v1.summary.histogram('scale%d_disp%d' % (s, i), self.disp[i][s])
          tf.compat.v1.summary.image('scale%d_disparity%d' % (s, i), self.disp[i][s])

      for key in self.warped_image[s]:
        tf.compat.v1.summary.image('scale%d_warped_image%s' % (s, key),
                         self.warped_image[s][key])
        tf.compat.v1.summary.image('scale%d_warp_mask%s' % (s, key),
                         self.warp_mask[s][key])
        tf.compat.v1.summary.image('scale%d_warp_error%s' % (s, key),
                         self.warp_error[s][key])
        if self.ssim_weight > 0:
          tf.compat.v1.summary.image('scale%d_ssim_error%s' % (s, key),
                           self.ssim_error[s][key])
        if self.icp_weight > 0:
          tf.compat.v1.summary.image('scale%d_icp_residual%s' % (s, key),
                           self.icp_residual[s][key])
          transform = self.icp_transform[s][key]
          tf.compat.v1.summary.histogram('scale%d_icp_tx%s' % (s, key), transform[:, 0])
          tf.compat.v1.summary.histogram('scale%d_icp_ty%s' % (s, key), transform[:, 1])
          tf.compat.v1.summary.histogram('scale%d_icp_tz%s' % (s, key), transform[:, 2])
          tf.compat.v1.summary.histogram('scale%d_icp_rx%s' % (s, key), transform[:, 3])
          tf.compat.v1.summary.histogram('scale%d_icp_ry%s' % (s, key), transform[:, 4])
          tf.compat.v1.summary.histogram('scale%d_icp_rz%s' % (s, key), transform[:, 5])

  def build_depth_test_graph(self):
    """Builds depth model reading from placeholders."""
    with tf.compat.v1.name_scope('depth_prediction'):
      with tf.compat.v1.variable_scope('depth_prediction'):
        input_uint8 = tf.compat.v1.placeholder(
