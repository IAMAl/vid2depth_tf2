...
def placeholder_inputs(batch_size):
  ...
  source_placeholder = tf.compat.v1.placeholder(tf.float32, shape=cloud_shape)
  target_placeholder = tf.compat.v1.placeholder(tf.float32, shape=cloud_shape)
...
def inference(source, target):
  ...
  tf.compat.v1.summary.scalar('tx', ego_motion[0])
  tf.compat.v1.summary.scalar('ty', ego_motion[1])
  tf.compat.v1.summary.scalar('tz', ego_motion[2])
  tf.compat.v1.summary.scalar('rx', ego_motion[3])
  tf.compat.v1.summary.scalar('ry', ego_motion[4])
  tf.compat.v1.summary.scalar('rz', ego_motion[5])
  tf.compat.v1.summary.scalar('res_height', res_height[0])

  dist_to_center = tf.norm(tensor=(source - RES_CENTER)[:, :, :2], axis=2,
                           keepdims=True)
...
def loss_func(transform, residual):
  return (tf.reduce_mean(input_tensor=tf.square(transform), name='transform_mean') +
          tf.reduce_mean(input_tensor=tf.square(residual), name='residual_mean'))
