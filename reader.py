...
class DataReader(object):
  ...
  def augment_image_colorspace(cls, image_seq):
    ...
    white = tf.ones([tf.shape(input=image_seq)[0], tf.shape(input=image_seq)[1]])
  ...
  def compile_file_list(self, data_dir, split, load_pose=False):
    ...
    '''
    frame = glob.glob(data_dir+'/*')
    subfolder = frame[0].split('/')[2]
    
    frame_ids = glob.glob(data_dir+'/'+subfolder+'/*')
    ...
    if '.jpg' in frame_ids:
        image_file_list = [
            os.path.join(data_dir, subfolder, frame_ids[i] + '.jpg')
            for i in range(len(frame_ids))
        ]
    if 'calib' in frame_ids:
        cam_file_list = [
            os.path.join(data_dir, subfolder, frame_ids[i] + '_cam.txt')
            for i in range(len(frame_ids))
        ]
    '''
