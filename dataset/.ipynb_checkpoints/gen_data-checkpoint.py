...
def _generate_data():
  ...
  with open(os.path.join(FLAGS.data_dir, 'train.txt'), 'w') as train_f:
    with open(os.path.join(FLAGS.data_dir, 'val.txt'), 'w') as val_f:
      logging.info('Generating data...')
      for index, frame_chunk in enumerate(frame_chunks):
        all_examples.clear()
        pool.map(_gen_example_star,
                 izip(frame_chunk, itertools.repeat(all_examples)))
